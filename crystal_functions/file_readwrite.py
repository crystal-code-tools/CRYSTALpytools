...
class Properties_input:
    ...
    def make_bands_block(self,k_path,n_kpoints,first_band,last_band,print_eig=0,print_option=1,
        ...
        if 'HighSymmKpath' in str(type(k_path)):
            ...
            print('EXITING: k_path type must be a list of list (k coordinates) or                a pymatgen HighSymmKpath object. %s selected' %type(k_path) )
            sys.exit(1)
    ...
    def make_doss_block(self,n_points=200,band_range=None,e_range=None,plotting_option=2,
        ...
        if band_range == None and e_range == None:
            ...
            print('EXITING: either the band_range argument or the e_range argument                do not match the required format (2 item list)')
            sys.exit(1)
...
def cry_combine_density(density1,density2,density3,new_density='new_density.f98',spin_pol=False):
    ...
    for i,j in enumerate(density1_data.ncf):
        ...
        if density1_data.ghost[density1_data.atom_shell[density1_data.la3[j-1]-1]-1] == 0 and             density1_data.ghost[density1_data.atom_shell[density1_data.la4[j-1]-1]-1] == 0:
...
#po = Properties_output('SURFRHOO.DAT')
