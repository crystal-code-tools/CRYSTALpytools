...
def plot_cry_doss(doss, color='blue', fermi='forestgreen', save_to_file=False, overlap=False, labels=None, figsize=None, linestl=None,
                  linewidth=1, title=None, beta='up', energy_range=None, dos_range=None, prj=None):
    """
    Deprecated
    """
    import warnings

    warnings.warn('This function is deprecated. Use plot_electron_dos.')
    return plot_electron_dos(doss, color=color, fermi=fermi,
                             save_to_file=save_to_file, overlap=overlap,
...
def plot_electron_dos(doss, color='blue', fermi='forestgreen', save_to_file=False, overlap=False, labels=None, figsize=None, linestl=None,
                      linewidth=1, title=None, beta='up', energy_range=None, dos_range=None, prj=None):
    """
    Plot electron density of states.

    Args:
        doss (DOSBASE): DOS obect generated by :code:`CRYSTALpytools.crystal_io.Properties_output.read_cry_doss`.
        color (str | list[str]): Color of plot lines. *List when overlap=True*
        fermi (str): Color of Fermi level line
        save_to_file (bool)
        overlap (bool): Plotting multiple lines into the same figure
        labels (str | list[str]): *List when overlap=True*
        title (str)
        beta (str): 'up' or 'down'
        prj (list)
    """
    ...
    if save_to_file != False:
        ...
        if path.exists(folder) == True:
            ...
            raise FileNotFoundError('Folder %s does not exist' % save_to_file)
